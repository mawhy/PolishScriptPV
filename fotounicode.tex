...
\makeindex[title=Indeks, intoc]
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    filecolor=magenta,      
    urlcolor=cyan,
    pdftitle={FOTOWOLTAIKA W BUDOWNICTWIE},
    bookmarks=true,
    }
...
materiału do wytwarzania napięcia, zwykle dzięki fotoemisji \index{fotoemisja}, w warunkach 
...
na ogół są zgrupowane w moduły fotowoltaiczne \index{moduł fotowoltaiczny} (znane także jako panele 
słoneczne). Moduły mogą być wzajemnie pogrupowane i połączone tworząc 
generatory fotowoltaiczne \index{generator fotowoltaiczny} (pola modułów). 
...
Głównym elementem takiego systemu jest \textit{\textbf{inwerter (falownik)}}. Inwerter \index{inwerter}
przemienia moc stałą \index{prąd stały} produkowaną przez panele PV na moc zmienną \index{prąd zmienny} zgodną z 
...
Układy autonomiczne \index{off-grid} są zaprojektowane tak, aby działać niezależnie od 
...
Taryfy gwarantowane \index{taryfy gwarantowane} są mechanizmem motywującym wprowadzonym przez rząd w 
...
Zielone certyfikaty \index{zielone certyfikaty} są zbywalnymi certyfikatami przyznawanymi przez 
...
słoneczne, 
\item Zintegrowane z dachem systemy PV \index{bipv} są na ogół droższe od systemów 
...
\textbf{Ogniwa polikrystaliczne}\index{ogniwo polikrystaliczne} są koloru niebieskiego z wyraźnie widocznymi 
...
\subsubsection{Generator PV, pole modułów PV} \index{generator fotowoltaiczny}- to kolekcja elektrycznie 
...
\textbf{STC} \index{STC} są zdefiniowane przy poziomie natężenia promieniowania słonecznego rzędu 1000 ${W/m^{2}}$ w temperaturze 25 stopni Celsjusza i atmosferycznej masie optycznej równej 1.5. Jest to szczytowe natężenie promieniowania i umiarkowana temperatura, co stanowi ,,idealne''  warunki działania dla modułu solarnego. W rzeczywistości, występujące natężenie promieniowania jest znacznie mniejsze niż wartość szczytowa, a występująca temperatura jest zazwyczaj większa niż ta na poziomie STC. 
...
\subsubsection{Moc szczytowa} \index{moc szczytowa}
...
powinna być orientacja południowa.\index{azymut} W każdym przypadku powinno się unikać 
...
dokładnie definicji prądu stałego DC: \index{prąd stały}
\begin{displayquote}
...
prądu, drugim istotnym parametrem charakteryzującym prąd AC.\index{prąd zmienny} 

\subsection{Charakterystyki elektryczne}


Urządzenia fotowoltaiczne mogą również operować w zakresie od obwodu 
otwartego (zerowe natężenie)\index{napięcie obwodu otwartego} do zwarcia (zerowe napięcie)\index{prąd zwarciowy}. Pomiędzy tymi 
...
Jest to \textbf{maksymalny punkt mocy} (Maximum Power Point). \index{maksymalny punkt mocy}
...
osiągało maksymalną wartość - \textbf{natężenie zwarcia}\index{natężenie zwarcia} Isc. Jest to prąd, który 
...
jest \textbf{napięcie obwodu otwartego} Voc. \index{napięcie obwodu otwartego}
...
Średnia ilość \pdfmarkupcomment[markup=Squiggly,color=green]{energii promieniowania}{Natężenie promieniowania słonecznego docierającego do górnych granic atmosfery określone jest przez stałą słoneczną. Wielkość ta jest zdefiniowana dla średniej odległości Ziemia-Słońce i wynosi około 1366,1 ${W/m^{2}}$}.
...
Najwyższą sprawność ogniw fotowoltaicznych mają: \textbf{\pdfmarkupcomment[markup=Squiggly,color=green]{ogniwa hybrydowe}{systemy produkujące energię elektryczną bądź cieplną, przy użyciu więcej niż jednego źródła energii. W przypadku fotowoltaiki są kombinacją panelu fotowoltaicznego i innego systemu wytwarzania energii (np. generatora spalinowego, turbiny wiatrowej, kolektora słonecznego itp.)}} \textit(kolektor słoneczny i panel fotowoltaiczny w jednym)

Jakiego typu diody zabezpieczają panele fotowoltaiczne połączone szeregowo przez wpływem prądu zwrotnego: \textbf{\pdfmarkupcomment[markup=Squiggly,color=green]{diody bocznikujące}{}}Diody bocznikujące zapobiegają termicznemu zniszczeniu paneli fotowoltaicznych
...
Maksymalna sprawność modułu fotowoltaicznego wynosi: \textbf{\pdfmarkupcomment[markup=Squiggly,color=green]{15-18 \%}{}} Obecnie produkowane ogniwa mają sprawność od 15% do 18%.
...
Wyjście paneli do inwertera jest:  \textbf{\pdfmarkupcomment[markup=Squiggly,color=green]{stałoprądowe DC}{}} Z paneli fotowoltaicznych wychodzi prąd stały (DC). Z inwertera do sieci elektroenergetycznej wychodzi prąd zmienny (przemienny) AC.
...
\textbf{Anoda} (ang. Anode)\index{anoda} – Element budowy akumulatora. Podczas pracy akumulatora ma znak ujemny, podczas ładowania dodatni. Na niej zachodzi proces utleniania.


\textbf{Bateria słoneczna} (ang. Solar array)\index{panel fotowoltaiczny} – zob. panel fotowoltaiczny

\textbf{BIPV} (ang. Buidling-Integrated Photovoltaics)\index{bipv} – integracja ogniw fotowoltaicznych z konstrukcjami budynków

\textbf{Bilans energetyczny} (ang. Energy balance)\index{bilans energetyczny} – jest to analiza przepływu energii zachodzącego podczas eksploatacji instalacji fotowoltaicznej.  Na bilans energetyczny składają się zapotrzebowanie energetyczne i efekt energetyczny.

\textbf{Bocznikujące diody} (ang. Bypass diode)\index{dioda bocznikująca} – element półprzewodnikowy stosowany w panelach fotowoltaicznych. Zabezpieczają one przed przepływem prądu zwrotnego\index{prąd zwrotny} przez niesprawne ogniwa fotowoltaiczne, przeprowadzając prąd wokół nich.

\textbf{Bezpośrednie promieniowanie} (ang. Direct radiation)\index{bezpośrednie promieniowanie} – część energii promieniowania słonecznego docierającego przez atmosferę do powierzchni ziemskiej bezpośrednio od Słońca. Ma postać promieni równoległych, nierozproszonych na żadnej przeszkodzie.

\textbf{Blokujące diody} (ang. blocking diode)\index{dioda blokująca} – tak jak diody bocznikujące są elementem zabezpieczającym panel fotowoltaiczny przed wpływem prądu zwrotnego. Ich działanie polega na zatrzymaniu przepływu prądu przez niesprawne ogniwo.


\textbf{Chalkopiryty} CuFeS2 (ang. Chalcopyrite)\index{chalkopiryt} – są to minerały z grupy siarczków, w fotowoltaice wykorzystywane jako półprzewodniki w ogniwach cienkowarstwowych. Najczęściej stosowany jest diselenek miedziowo indowy. Ogniwa wykorzystujące chalkopiryty wykazują w warunkach laboratoryjnych sprawności rzędu 18\%, jednak technologia ich wytwarzania jest stosunkowo droga.

\textbf{Czysta energia} (ang. Clean Energy)\index{energia odnawialna}, energia odnawialna – Energia powstała przez wykorzystanie odnawialnych źródeł energii.

\textbf{Charakterystyka prądowa-napięciowa} (ang. voltage-current characteristic)\index{charakterystyka prądowa-napięciowa} – wykres przedstawiający zależność pomiędzy natężeniem prądu a napięciem w panelu fotowoltaicznym, od punktu bez obciążenia do maksymalnego napięcia. Krzywa wykresu obrazuje wydajność baterii słonecznej.

\textbf{Całkowite promieniowanie} (ang. total radiation)\index{całkowite promieniowanie} – jest to suma całkowitej ilości światła pochodzącego ze słońca dochodzącego do powierzchni ziemi – zestawienie wartości promieniowania bezpośredniego, rozproszonego i odbitego.

\textbf{Cienkowarstwowe ogniwa} (ang. Thin-film solar cells)\index{cienkowarstwowe ogniwa} – należą do ogniw drugiej generacji, wykonane są poprzez nanoszenie cienkich warstw krzemu amorficznego na szklane płytki. Stanowią one obiecującą alternatywę dla ogniw I generacji. Dają możliwość umieszczania na rozmaitych obiektach, również na elastycznych. Obecnie efektywność produkcji energii jest niższa niż przy zwykłych ogniwach, jednakże niższe są również koszty produkcji.

\textbf{CISG} (ang. Copper indium gallium selenide)\index{CISG} – materiał półprzewodnikowy zbudowany z miedzi, indu, galu i selenu. Jest to typ ogniw cienkowarstwowych charakteryzujących się wyższą efektywnością i trwałością. Należą one do ogniw II generacji.

\textbf{Ciemny prąd} (ang. dark current)\index{ciemny prąd}\index{prąd zwarciowy} – jest to prąd o niskiej wartości przepływający przez urządzenia światłoczułe, w czasie braku dostępu światła.


\textbf{Diody obejściowe} (ang. Bypass Diode)\index{dioda bocznikująca} – zobacz bocznikujące diody

\textbf{Diody blokujące} (ang. Blocking Diode)\index{dioda blokująca} – działają tak samo jak diody obejściowe (bocznikujące). Różnią się położeniem w instalacji fotowoltaicznej.

\textbf{DC} (ang. Direct current)\index{prąd stały} – prąd stały charakteryzuje się stałą wartością kierunku i natężenia przepływu prądu. Przy zasilaniu prądem stałym chwilowa wartość mocy jest stała, co jest konieczne dla układów wzmacniania i przetwarzania sygnałów, oraz dla półprzewodnikowych układów elektronicznych.

\textbf{DSSC} (ang. Dye-sensitized solar cell)\index{DSSC} – innowacyjna technologia w ogniwach fotowoltaicznych III generacji oparta na zjawisku konwersji fotochemicznej. Analogicznie do zjawiska fotosyntezy zachodzącej w roślinach, w ogniwach DSSC odpowiednikiem chlorofilu są barwniki syntetyczne, natomiast samo ogniwo zbudowane jest z nanostruktury TiO2. Pomiędzy barwnikiem a powierzchnią przewodzącą znajduje się elektrolit na bazie jodku. Ogniwa DSSC jak wszystkie ogniwa III generacji należą do urządzeń o niskiej sprawności.
...
\textbf{Efektywność} (ang. Efficient)\index{efektywność} – jest to stosunek energii (mocy) wyjściowej do energii (mocy) wejściowej panelu fotowoltaicznego. Wyrażana jest ona w procentach.
...
\textbf{Farma fotowoltaiczna} (ang. Photovoltaic farm, PV farm) \index{farma fotowoltaiczna}– naziemna bądź dachowa instalacja fotowoltaiczna o dużej mocy

\textbf{Fotowoltaika} (ang. Photovoltaic, PV)\index{fotovoltaika} – dziedzina nauki zajmująca się konwersją promieniowania słonecznego na energię elektryczną przy wykorzystani zjawiska (efektu) fotowoltaicznego.

\textbf{Fotoogniwo} (ang. Solar cell, photovoltaic cell)\index{ogniwo} – element półprzewodnikowy, w którym następuje konwersja fotowoltaiczna. Ogniwo jest podstawową jednostką budowy panelu fotowoltaicznego.

\textbf{Foton} (ang. Photon)\index{foton} – jest to cząstka elementarna nieposiadająca ładunku elektrycznego ani momentu magnetycznego. Fotony są nośnikami oddziaływań elektromagnetycznych.


\textbf{Fotoemisja} (ang. Photoelectric effect)\index{fotoemisja} – emisja fotoelektronowa – jest to zjawisko emitowania elektronów przez obiekty, w wyniku oświetlania ich światłem, którego energia jest wyższa niż energia wzbudzenia


\textbf{Falownik} (ang. Inverter)\index{inwerter} \label{falownik} \hypertarget{inwerter}– przekształtnik prądu stałego na prąd zmienny (DC -> AC). W instalacjach fotowoltaicznych przekształca prąd stały powstały w bateriach słonecznych i dostosowuje go do odbiorników i sieci elektrycznej. Falownik obok paneli jest drugim najistotniejszym elementem instalacji PV.

\textbf{Generacja energii elektrycznej} (ang. Generation of electricity) – wytwarzanie energii elektrycznej

\textbf{Gęstość strumienia energii} (ang.)\index{stała słoneczna} – zob. natężenie promieniowania słonecznego, stała słoneczna

\textbf{Generacje ogniw} (ang. Solar cells generation)\index{generacje ogniw} – systematyka wprowadzona w celu podziału ogniw fotowoltaicznych w zależności od technologii i materiału ich  wykonania. Wyróżniamy 3 generacje, o uproszczonej charakterystyce:


\textbf{Hybrydowe systemy} (ang.)\index{hybrydowe systemy} – systemy produkujące energię elektryczną bądź cieplną, przy użyciu więcej niż jednego źródła energii. W przypadku fotowoltaiki są kombinacją panelu fotowoltaicznego i innego systemu wytwarzania energii (np. generatora spalinowego, turbiny wiatrowej, kolektora słonecznego itp.)

\textbf{Heterozłączowe ogniwo} (ang. Heterojunction cell)\index{ogniwo heterozłączowe} – typ złącza w półprzewodniku, mamy z nim do czynienia, gdy przewodzenie p – n występuje na styku dwóch materiałów o różnym typie przewodnictwa.
...
\textbf{Inwerter} (ang. Inverter)\index{inewerter} – przetwornica/przetwornik/falownik – element budowy instalacji fotowoltaicznej, urządzenie służące do zamiany prądu stałego na prąd zmienny. Zob. falownik.

\textbf{ISC} (ang. Short circuit current)\index{prąd zwarciowy} – prąd zwarciowy – wartość natężenia prądu w ogniwie fotowoltaicznym, w chwili maksymalnego obciążenia.

\textbf{[J] – dżul} (ang. Joule)\index{J dżul} – jednostka pracy, energii oraz ciepła w układzie SI równa 1/3600 kWh (kilowatogodzin). 1J to praca wykonana przez siłę o wartości 1N przy przesunięciu punktu przyłożenia siły o 1 m w kierunku równoległym do kierunku działania siły.

\textbf{Jałowe napięcie} (ang. Open circuit voltage)\index{jałowe napięcie} – napięcie prądu w panelu fotowoltaicznym w momencie niepodłączenia do żadnego obciążenia.

\textbf{Katoda} (ang. Cathode)\index{katoda} – element budowy urządzenia elektrycznego, przez który wypływa prąd elektryczny. Katoda zawsze występuje w parze z elektrodą ujemną – anodą, przez którą prąd wpływa.

\textbf{Konwerter} (ang. Converter)\index{konwerter} – jest to urządzenie elektryczne służące do przemiany prądu zmiennego w prąd stały.

\textbf{Krzem} (ang. Silicon)\index{krzem} – pierwiastek chemiczny o wzorze Si, główny materiał budulcowy ogniw fotowoltaicznych.  Krzemowe ogniwa fotowoltaiczne należą do ogniw I generacji i charakteryzują się najwyższymi osiąganymi sprawnościami. Problemem jest jednak cena owych ogniw. Krzem do celów fotowoltaicznych musi być w 99,9\% w czystej postaci. Najefektywniejsze jest wykorzystanie ogniw z krzemu monokrystalicznego. Występują także tańsze i mniej trwałe odmiany ogniw krzemowych i są to: polikrystaliczne i amorficzne (bezpostaciowe).

\textbf{Krzywa mocy} (ang. Power curve) \index{krzywa mocy} – jest to wykres zależności natężenia prądu od napięcia w panelu fotowoltaicznym. Analiza krzywej mocy służy np. odnalezieniu punktu maksymalnej mocy.

\textbf{Konwersja fotowoltaiczna} (ang. Photovoltaic conversion)\index{konwersja fotowoltaiczna}  – bezpośrednia zmiana energii promieniowania słonecznego na energię elektryczną zachodząca w ogniwie fotowoltaicznym.

\textbf{Kąt pochylenia} (ang. Tilt angle)\index{kąt pochylenia} – kąt nachylenia panelu słonecznego względem słońca mierzony od poziomu. Nachylenie panelu wpływa na efektywność wytwarzania energii elektrycznej i powinna być dostosowana do kąta, pod jakim padają promienie słoneczne. W ciągu roku, gdy słońce znajduje się na różnych wysokościach, odpowiedni kąt nachylenia waha się od 30° d0 60°. Optymalnym całorocznym ustawieniem paneli na obszarze Polski jest kąt 40°.

\textbf{Kąt padania} (ang. Angle of Incidence)\index{kąt padania} – Jest to kąt, przy którym promienie słoneczne padające na powierzchnię panelu są do niego prostopadłe. Kąt padania wynosi 0°, gdy powierzchnia panelu skierowana jest bezpośrednio w stronę Słońca. Kąt padania jest równy 90°, gdy oświetlana powierzchnia jest równoległa do promieni słonecznych.

\textbf{Moduł} (ang. Module)\index{moduł} – Integralne, hermetycznie zamknięte urządzenie, składające się z ogniw fotowoltaicznych, zdolne do wytwarzania prądu stałego pod wpływem promieniowania świetlnego.

\textbf{Moc} (ang. Power)\index{moc} – skalarna wielkość fizyczna określająca pracę wykonaną w jednostce czasu przez układ fizyczny. Podawana przez producenta nominalna moc modułu fotowoltaicznego oznacza moc zmierzoną w warunkach testowych (STC). Zależy ona od natężenia promieniowania słonecznego, wyrażana jest w jednostce ${W/m^{2}}$.

\textbf{Monokrystaliczny krzem} (ang. Monokrystalline silicon)\index{monokrystaliczny krzem} – materiał budulcowy podstawowego i jak dotąd najsprawniejszego typu ogniw fotowoltaicznych. Ogniwa takie zbudowane są z jednorodnego kryształu krzemu o uporządkowanej budowie wewnętrznej. Powstają poprzez cięcie bloku krzemu o odpowiedniej wielkości, na „plastry” grubości ok. 0,3 mm. Monokrystaliczny krzem powstaje poprzez roztopienie krzemu polikrystalicznego, a następnie w powolnym procesie krzepnięcia.
...
\textbf{Moc zainstalowana} (ang. Istalled capacity) \index{moc zainstalowana}– określa wartość potencjalnej wielkości energii elektrycznej możliwej do uzyskania przez instalację fotowoltaiczną (inaczej – moc dyspozycyjna zainstalowanych urządzeń).

\textbf{Napięcie elektryczne} (ang. Voltage)\index{napięcie elektryczne} – różnica potencjałów elektrycznych między dwoma punktami obwodu elektrycznego lub pola elektrycznego. Wartości charakterystyczne napięcia sieciowego w Polsce określa Norma  PN-IEC 60038 i wynoszą: 50Hz i 230V.

\textbf{Natężenie promieniowania słonecznego} (ang. Intensity of solar radiation), \index{natężenie promieniowania słonecznego} inaczej gęstość strumienia energii – jest to ilość energii cieplnej, jaką otrzymuje jednostka powierzchni ustawiona prostopadle do biegu promieni, w jednostce czasu. Oznaczamy ją symbolem I. Zob. też: Stała słoneczna.

\textbf{Natężenie prądu} \index{natężenie prądu}– liczba ładunków przepływających z prędkością v, przez powierzchnię s (przekrój przewodnika). W układzie SI jednostką natężenia prądu jest amper [A] (zob. Amper).

\textbf{On-Grid system} \index{on-grid} – system fotowoltaiczny przyłączony do lokalnej sieci energetycznej.

\textbf{Odbite promieniowanie} \index{odbite promieniowanie} – to ta część promieniowania całkowitego, które dochodząc do powierzchni Ziemi jest odbijana ku górze.

\textbf{OZE}\index{OZE} –  Jest to skrótowe określenie na „odnawialne źródła energii”, czyli takie, których użytkowanie nie wiąże się ze zwiększaniem ich deficytu. Do odnawialnych źródeł energii zaliczamy energię masy ziemi, słoneczną, wiatrową, oraz energię wody zarówno powierzchniowej jak i głębinowej (rzeki, jeziora, cieki geotermalne itp.).

\textbf{OPV} (ang. Organic photovoltaic) \index{OPV}– organiczne ogniwa fotowoltaiczne – ogniwa III generacji


\textbf{Prąd elektryczny} (ang. Electric current) – uporządkowany ruch ładunków elektrycznych.

\textbf{Przemienny prąd AC} (ang. Alternating current) \index{prąd przemienny} – jest jednym z rodzajów prądów zmiennych (zob. AC), charakteryzujący się okresowymi zmianami napięcia elektrycznego z określoną częstotliwością. Wartości chwilowe prądu przyjmują wartości na przemian dodatnie i ujemne. Pożądanym jest by wartość średnia całookresowa wynosiła 0. Zakłócenia lub nieliniowość powodują powstanie przebiegu odkształconego. Urządzenia elektryczne codziennego użycia pracują z wykorzystaniem prądu zmiennego. (zob też: Napięcie elektryczne)

\textbf{Polimery} \index{polimery} – z naprzemiennymi wiązaniami chemicznymi pojedynczymi i podwójnymi mają strukturę podobną do półprzewodnika (polimery przewodzące). Są bardzo tanie w produkcji, elastyczne. Sprawności rzędu 3 – 5 \%.
...
\textbf{Promieniowanie słońca} (ang. solar radiation) \index{promieniowanie słońca}– jest to strumień fal elektromagnetycznych i cząstek elementarnych docierający ze Słońca do Ziemi główne źródło energii cieplnej docierającej do Ziemi ze Słońca

\textbf{Pochłonięte promieniowanie} (ang. )\index{pochłonięte promieniowanie} – jest to różnica pomiędzy promieniowaniem całkowitym, a odbitym. Pochłanianie powoduje zmianę jakościową energii słonecznej, dzięki niemu jej część przekształca się w energię cieplną


\textbf{PVT} (ang. Photovoltaic thermal)\index{kolektory hybrydowe} – kolektory hybrydowe – połączenie paneli fotowoltaicznych z kolektorami słonecznymi
...
\textbf{Solar} (ang. Solar)\index{panel fotowoltaiczny} – panel fotowoltaiczny

\textbf{Słoneczne ogniwo} (ang.Photovoltaic cell)\index{ogniwo} – najmniejsza część płyty panelu fotowoltaicznego

\textbf{STC} – standard test conditions (ang. )\index{STC} – standardowe warunki testowania

\textbf{Stały prąd} (ang. Direct current)\index{stały prąd} – zobacz DC

\textbf{Sinusoidalny prąd} (ang. Sinusoidal current)\index{przemienny prąd} – zob. przemienny prąd
...
\textbf{System PV z połączeniem do sieci energetycznej} (ang. On-grid instalation) \index{on-grid} – Instalacja fotowoltaiczna, która nadwyżki wyprodukowanej energii (lub całą jej wartość) przesyła do lokalnej sieci energetycznej


\textbf{Telurek kadmu CdTe} \index{telurek kadmu}– technologia bardzo prosta. Pracuje w złączu z siarczkiem kadmu CdS, jednak z racji znaczącej szkodliwości (trujący) nie jest akceptowalny społecznie.

\textbf{Temperatura otoczenia} (ang. Ambient temperature)\index{temperatura otoczenia} – temperatura na zewnątrz instalacji.

\textbf{Voc} (ang. Open circuit voltage)\index{napięcie obwodu otwartego} – Napięcie obwodu otwartego/napięcie jałowe

\textbf{Wat} (ang. watt) [W] \index{Wat}– jest to główna jednostka czynnej mocy elektrycznej w układzie SI. Oznacza moc, dla której praca wykonana w czasie jednej sekundy równa jest jednemu dżulowi.

\textbf{Wyspowy system} (ang. Off-grid)\index{off-grid} – zob. autonomiczny system


\textbf{ABSORPCJA ŚWIATŁA}\index{absorpcja światła}
...
\textbf{AKUMULATOR}\index{akumulator}
...
\textbf{AMORFICZNE OGNIWA FOTOWOLTAICZNE}\index{amorficzne ogniwa fotowoltaiczne}
...
\textbf{DIODA OBEJŚCIOWA} (dioda bocznikowa)\index{dioda bocznikowa}
...
\textbf{EVA} laminat\index{laminat}
...
\textbf{FALOWNIK}\index{falownik} \hypertarget{falownik}
...
\textbf{FOTOWOLTAIKA} (PV)\index{fotowoltaika}
...
\textbf{INSTALACJA ELEWACYJNA}\index{instalacja elewacyjna}
...
\textbf{KĄT AZYMUTU}\index{kąt azymutu}
...
\textbf{KĄT NACHYLENIA}\index{kąt nachylenia}
...
\textbf{MAXIMUM POWER POINT} (MPP)\index{maximum power point}
...
\textbf{MODUŁY SŁONECZNE}\index{moduły słoneczne}
...
\textbf{NAPROWADZANIE} \index{naprowadzanie}
